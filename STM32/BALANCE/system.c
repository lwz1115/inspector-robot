...
u8 ROS_Lock_Flag=0;  // 切换到ROS模式后锁定，阻止PS2 Start键重新激活
...
//	uart5_init(115200);
	...
	//ADC引脚初始化，用于读取电池电压和电位器档位，电位器档位决定小车启动后的小车型号
 	Adc_Init();  
	Adc_POWER_Init();
  ...
  //根据电位器的档位判断需要匹配哪一型号的小车，然后对对应的参数初始化
	Robot_Select();                 
  ...
  //编码器C初始化，用于读取电机B的实时速度
   ...
   // TIM8_SERVO_Init(9999,168-1);//APB2的时钟频率为168M , 频率=168M/((9999+1)*(167+1))=100Hz
	 ...
	 //初始化航模遥控接口
	 //TIM8_Cap_Init(9999,168-1);  //高级定时器TIM8时钟频率为168M    

  //Initialize motor speed control and, for controlling motor speed, PWM frequency 10kHz
  //初始化电机速度控制，用于控制电机速度，PWM频率10KHZ
  //APB2时钟频率为168M，PWM为16799，频率=168M/((16799+1)*(0+1))=10k
  ...
  //MPU6050 初始化，用于读取小车三轴姿态、三轴角速度、三轴加速度信息
   MPU6050_initialize();        		
	...
	PS2_Init();
	PS2_SetInit();
	DHT11_Init();      // 温湿度传感器 PA5
	Adc_Smoke_Init();  // 烟雾传感器 PA6
}
