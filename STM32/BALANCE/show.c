...
Input   : none
Output  : none
功能描述：读取电池电压、蜂鸣器报警、启动自检、向APP发送数据、OLED显示任务
入口参数：无
返回  值：无
**************************************************************************/
int Buzzer_count=25;
void show_task(void *pvParameters)
{
   ...
   {	
		...
		vTaskDelayUntil(&lastWakeTime, F2T(RATE_10_HZ)); //This task runs at 10Hz //该任务以10Hz的频率运行
		...
		if(Buzzer_count<5)Buzzer=1; //The buzzer is buzzing //蜂鸣器鸣叫中
		...
		else if(10<=Voltage && Voltage<10.5f && LowVoltage_1<2)LowVoltage_1++; //10.5V低电量时蜂鸣器第一次报警
   ...
   static int count=0;	 
	 ...
	 {	
		 ...
		 else if(Car_Mode==Akm_Car||Car_Mode==Diff_Car||Car_Mode==FourWheel_Car||Car_Mode==Tank_Car)
		 {
			 ...
			 //阿克曼、差速、四驱、坦克车显示陀螺仪零漂
			 OLED_ShowString(55,0,"BIAS");
			 if( Deviation_gyro[2]<0)  OLED_ShowString(90,0,"-"),OLED_ShowNumber(100,0,-Deviation_gyro[2],3,12);  //Zero-drift data of gyroscope Z axis //陀螺仪z轴零漂数据
		 ...
		 else if(Car_Mode==Akm_Car||Car_Mode==Diff_Car||Car_Mode==Tank_Car)
		 {
			 ...
			 //阿克曼、差速、坦克车显示电机A（左）的目标速度和当前实际速度
			 ...
			 //阿克曼、差速、坦克车显示电机B（右）的目标速度和当前实际速度
