...
Output  : none
功能描述：逆运动学解算，根据三轴目标速度计算各轮目标转速
入口参数：X、Y、Z轴方向的目标移动速度
返回  值：无
**************************************************************************/
void Drive_Motor(float Vx,float Vy,float Vz)
{
	  ...
	  if (Car_Mode==Mec_Car) 
    {
		...
		{
			//Inverse kinematics //逆运动学解算
			MOTOR_A.Target  = Vx - Vz * Wheel_spacing / 2.0f; //左电机目标速度
		  MOTOR_B.Target =  Vx + Vz * Wheel_spacing / 2.0f; //右电机目标速度
		...
		{	
			//Inverse kinematics //逆运动学解算
			MOTOR_A.Target  = Vx - Vz * (Wheel_spacing +  Axle_spacing) / 2.0f; //左前电机目标速度
			MOTOR_B.Target  = Vx - Vz * (Wheel_spacing +  Axle_spacing) / 2.0f; //左后电机目标速度
			MOTOR_C.Target  = Vx + Vz * (Wheel_spacing +  Axle_spacing) / 2.0f; //右前电机目标速度
			MOTOR_D.Target  = Vx + Vz * (Wheel_spacing +  Axle_spacing) / 2.0f; //右后电机目标速度
...
{ 
    ...
    {	
			...
			//获取编码器数据，即实时轮速，并转换单位为国际单位
			Get_Velocity_Form_Encoder();   
			
			if(Check==0) //If self-check mode is not enabled //如果没有开启自检模式
			{
				//PS2 Start key (PS2_KEY==4): switch to PS2 control mode from any mode
				//PS2 Start键：在任意模式下切换到PS2控制模式
				if(PS2_KEY==4)
				{
					PS2_ON_Flag=1;
					APP_ON_Flag=0;
				...
				//CAN、串口1、串口3(ROS)、串口5控制可直接得到三轴目标速度，无需额外处理
        ...
        //and the software failure flag is 0
           ...
           //Speed closed-loop control to calculate the PWM value of each motor, 
					 ...
					 switch(Car_Mode)
					 {
							...
							case Diff_Car:      Set_Pwm( MOTOR_A.Motor_Pwm,  MOTOR_B.Motor_Pwm,  MOTOR_C.Motor_Pwm, MOTOR_D.Motor_Pwm, 0    ); break; //Differential car        //差速驱动小车
							...
							case Tank_Car:      Set_Pwm( MOTOR_A.Motor_Pwm,  MOTOR_B.Motor_Pwm,  MOTOR_C.Motor_Pwm, MOTOR_D.Motor_Pwm, 0    ); break; //Tank Car                //坦克车
					 }
				 }
				 //If Turn_Off(Voltage) returns to 1, the car is not allowed to move, and the PWM value is set to 0
				 //如果Turn_Off(Voltage)返回值为1，则不允许小车运动，PWM值设置为0
...
功能描述：检查电池电压、使能开关状态、软件失能标志位状态
入口参数：电压
返回  值：是否允许控制，1：不允许，0：允许
**************************************************************************/
...
{
	u8 Flag_Move=1;
	if(Car_Mode==Mec_Car||Car_Mode==Omni_Car) //The omnidirectional wheel moving trolley can move laterally //全向轮移动小车可以进行横向移动
	{
	 ...
	 {	
		 ...
		 //如果无方向控制指令，检查转向控制状态
		 if     (Flag_Left ==1)  Move_Z= PI/2*(RC_Velocity/500); //left rotation  //向左转
		 else if(Flag_Right==1)  Move_Z=-PI/2*(RC_Velocity/500); //right rotation //向右转
	...
	else //Non-omnidirectional moving trolley //非全向移动小车
	{
	 ...
	 if     (Flag_Left ==1)  Move_Z= PI/2; //left rotation  //向左转
	 else if(Flag_Right==1)  Move_Z=-PI/2; //right rotation //向右转
	}
...
{
   	int LX,LY,RY;
		int Threshold=20; //Threshold to ignore small movements of the joystick //阈值，忽略摇杆小幅度抖动
	  ...
	  //Ignore small movements of the joystick //忽略摇杆小幅度抖动
	  ...
	  else if(PS2_KEY==1)   //SELECT key: switch to ROS serial control mode //SELECT键：切换到ROS串口控制模式
	  {
	    PS2_ON_Flag=0;
	    APP_ON_Flag=0;
	    Remote_ON_Flag=0;
...
void Remote_Control(void)
{
    ...
    int Threshold=100; //Threshold to ignore small movements of the joystick //阈值，忽略摇杆小幅度抖动
    ...
    LX=Remoter_Ch2-1500; 
	  ...
	  //左摇杆左右方向。控制左右移动，只有全向轮才会使用该通道。阿克曼小车使用该通道作为PWM输出控制舵机
    LY=Remoter_Ch4-1500;
...
Input   : none
Output  : none
功能描述：防止电位器选择错误模式，导致初始化错误引起电机旋转，停止使用
入口参数：无
返回  值：无
**************************************************************************/
void robot_mode_check(void)
{
	...
	//当目标速度为0时，任意电机PWM超过2500，说明模式选择有误
