...
void data_transition(void)
{
	...
	//根据不同车型选择不同运动学算法进行正运动学解算，从各轮速度得到三轴速度
	switch(Car_Mode)
	{	
		case Mec_Car:      
	...
	Send_Data.Sensor_Str.Accelerometer.X_data= accel[1]; //The accelerometer Y-axis is converted to the ros coordinate X axis //加速度计Y轴转换为ROS坐标X轴
	Send_Data.Sensor_Str.Accelerometer.Y_data=-accel[0]; //The accelerometer X-axis is converted to the ros coordinate y axis //加速度计X轴转换为ROS坐标Y轴
	Send_Data.Sensor_Str.Accelerometer.Z_data= accel[2]; //The accelerometer Z-axis is converted to the ros coordinate Z axis //加速度计Z轴转换为ROS坐标Z轴
	
	//The Angle velocity of the triaxial velocity //陀螺仪三轴角速度
	Send_Data.Sensor_Str.Gyroscope.X_data= gyro[1]; //The Y-axis is converted to the ros coordinate X axis //陀螺仪Y轴转换为ROS坐标X轴
	Send_Data.Sensor_Str.Gyroscope.Y_data=-gyro[0]; //The X-axis is converted to the ros coordinate y axis //陀螺仪X轴转换为ROS坐标Y轴
	if(Flag_Stop==0) 
	...
	//电池电压（这里将浮点数放大了一千倍传输，对应在接收端接收到数据后也要缩小一千倍）
  ...
  Send_Data.buffer[1]=Flag_Stop;
	...
	// 校验 [26], 帧尾 [27-29 padding + tail at 29]
...
{
  unsigned char i = 0;	
	for(i=0; i<30; i++)  //发送完整30字节帧（含温湿度烟雾校验帧尾）
	{
...
{  	 
  ...
  //USART Initialization Settings //初始化设置
	...
	USART_ITConfig(USART1, USART_IT_RXNE, ENABLE); //Open the serial port to accept interrupts //开启串口接收中断
...
{  	 
  ...
  //Initialize the VIC register with the specified parameters 
	...
	USART_ITConfig(USART2, USART_IT_RXNE, ENABLE); //Open the serial port to accept interrupts //开启串口接收中断
...
{  	 
  ...
  USART_ITConfig(USART3, USART_IT_RXNE, ENABLE); //Open the serial port to accept interrupts //开启串口接收中断
...
{  	 
  ...
  USART_ITConfig(UART5, USART_IT_RXNE, ENABLE); //Open the serial port to accept interrupts //开启串口接收中断
    ...
    Last_Usart_Receive=Usart_Receive;			
      ...
      // Enter the APP direction control interface		
		...
		else if(Turn_Flag==1)
		{
			...
			//APP转向控制界面命令
			if     (Usart_Receive==0x43) Flag_Left=0,Flag_Right=1; //Right rotation //向右转
			else if(Usart_Receive==0x47) Flag_Left=1,Flag_Right=0; //Left rotation  //向左转
	 ...
	 if(Flag_PID==2) //Analyze the data //分析数据
	 {
      ...
      //相关标志位清零
			...
			j=0;
			Data=0;
			memset(Receive, 0, sizeof(u8)*50); //Clear the array to zero //清空数组
	 }
...
功能描述：将目标前进速度Vx、目标角速度Vz，转换为阿克曼小车的前进和转向角
入口参数：目标前进速度Vx、目标角速度Vz，单位：m/s、rad/s
返回  值：阿克曼小车的前进转向角，单位：rad
**************************************************************************/
...
Input   : 8 bits high, 8 bits low
Output  : The target velocity of the robot on the X/Y/Z axis
功能描述：将单位还原，将数据的高8位和低8位重新合成一个short类型数据后进行单位还原
入口参数：高8位、低8位
返回  值：机器人X/Y/Z轴的目标速度
**************************************************************************/
...
Output  : Check result
功能描述：计算要发送/接收的数据校验位
入口参数：Count_Number：校验前几位数据；Mode：0-对接收数据进行校验，1-对发送数据进行校验
返回  值：校验结果
**************************************************************************/
